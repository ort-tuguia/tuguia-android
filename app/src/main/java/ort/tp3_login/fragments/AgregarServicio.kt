    ...
    override fun onStart() {
        ...
        buttonAgregar.setOnClickListener {
            ...
            fun savePictureOnFirebase() {
                ...
                var urlPhoto: String = ""
                storageReference =
                    FirebaseStorage.getInstance().reference.child("images/activities/$fileName")
                if (viewModel.servicioUrlFoto.toString() != null) {
                    storageReference.putFile(viewModel.servicioUrlFoto!!).addOnSuccessListener {
                        Toast.makeText(context, "Imagen subida correctamente", Toast.LENGTH_LONG)
                            .show()
                ...
                MaterialAlertDialogBuilder(view1.context)
                    ...
                    { dialog, which ->
                        arraylistCategorias?.forEach {
                            viewModel.servicioCategoriaId =
                                arraylistCategorias?.get(which)?.id.toString()
                        }
                    }.show()
            }
                ...
                responseLiveData.observe(viewLifecycleOwner, Observer {
                    ...
                    if (categoriasList != null) {
                        ...
                        viewModel.categorias =
                            arraylistCategorias?.let {
                                Array(it.size) { i ->
                                    arraylistCategorias!!.get(
                                        i
                                    ).name
                                }
                            }
